...
GEOMETRY_ORDER = ("scale", "rotate", "translate", "perspective", "crop")
APPEARANCE_ORDER = ("brightness_contrast", "gamma", "hsv", "blur", "noise", "jpeg", "shadow")
OCCLUSION_ORDER = ("cutout",)
...
def sample_geometry_matrix(
    ...
    for name in GEOMETRY_ORDER:
        ...
        if name == "scale":
            ...
            dx = rng.uniform(float(op_cfg["range_ratio"][0]), float(op_cfg["range_ratio"][1])) * image_width
            dy = rng.uniform(float(op_cfg["range_ratio"][0]), float(op_cfg["range_ratio"][1])) * image_height
...
def apply_occlusion_ops(
    ...
    for name in OCCLUSION_ORDER:
        ...
        if name == "cutout":
            ...
            meta["cutout_regions"] = [
                {"x1": region.x1, "y1": region.y1, "x2": region.x2, "y2": region.y2} for region in regions
            ]
...
def generate_augmented_sample(
    ...
    *,
    ...
    matrix, geometry_meta = sample_geometry_matrix(annotation, config["geometry"], template_cfg, rng)
    ...
    metrics, reject_reason = validate_augmented_sample(
        ...
        image_width=annotation.image_width,
        image_height=annotation.image_height,
        source_visible_keypoints=count_visible_keypoints(annotation.object_annotation.keypoints),
